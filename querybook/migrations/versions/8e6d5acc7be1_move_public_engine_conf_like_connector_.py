...
def upgrade():
    ...
    op.add_column(
        "query_engine", sa.Column("feature_params", sa.JSON(), nullable=False)
    )
    ...
    for engine in engines:
        if not engine.status_checker:
            continue
        engine.feature_params = {"status_checker": engine.status_checker}
    session.commit()
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column(
        "query_engine", sa.Column("control_params", mysql.JSON(), nullable=False)
    )
    op.add_column(
        "query_engine",
        sa.Column("status_checker", mysql.VARCHAR(length=255), nullable=True),
    )
