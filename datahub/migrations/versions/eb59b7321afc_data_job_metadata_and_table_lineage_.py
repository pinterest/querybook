...
def upgrade():
    ...
    op.add_column('data_job_metadata', sa.Column('metastore_id', sa.Integer(), nullable=True))
    op.create_foreign_key('job_metadata_metastore_fk', 'data_job_metadata', 'query_metastore', ['metastore_id'], ['id'])
    op.create_unique_constraint('unique_table_lineage', 'table_lineage', ['table_id', 'parent_table_id', 'job_metadata_id'])
    op.drop_constraint('table_lineage_ibfk_1', 'table_lineage', type_='foreignkey')
    op.create_foreign_key('table_lineage_table_fk', 'table_lineage', 'data_table', ['table_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key('table_lineage_job_metadata_fk', 'table_lineage', 'data_job_metadata', ['job_metadata_id'], ['id'], ondelete='CASCADE')
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint('table_lineage_job_metadata_fk', 'table_lineage', type_='foreignkey')
    op.drop_constraint('table_lineage_table_fk', 'table_lineage', type_='foreignkey')
    op.create_foreign_key('table_lineage_ibfk_1', 'table_lineage', 'data_job_metadata', ['job_metadata_id'], ['id'])
